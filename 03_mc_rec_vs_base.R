# ==========================================
# scripts/03_mc_rec_vs_base.R
# Monte Carlo: baseline vs Recolección+ (bandas visibles)
# ==========================================
...
# --- Monte Carlo ---
mc_runs <- 50# usa 500 para bandas suaves
set.seed(1234)
sdlog_params <- list(   # mayor incertidumbre para que se noten
  rho_rec  = 0.12,
  eps_sort = 0.10,
  eps_rec  = 0.10,
  tau_life = 0.20
...
mc_rec  <- run_mc_scenario(S0, P, time, vt_rec,  mc_runs, sdlog_params, "Recolección+")

# Exportes
write.csv(mc_base$bands, "resultsEC/baseline_kpis_bands_mc.csv", row.names = FALSE)
write.csv(mc_rec$bands,  "resultsEC/esc_rec_kpis_bands_mc.csv",  row.names = FALSE)
sum_mc <- rbind(mc_base$summary, mc_rec$summary)
write.csv(sum_mc, "resultsEC/kpis_resumen_mc_rec.csv", row.names = FALSE)
...
plot_bands(mc_base$bands, "figsEC/baseline_kpis_bands_mc_rec.png", main="Baseline")
plot_bands(mc_rec$bands,  "figsEC/esc_rec_kpis_bands_mc.png",    main="Recolección+")

cat("MC Recolección+ listo: bands en resultsEC/, figs en figsEC/, resumen en kpis_resumen_mc_rec.csv\n")
